...
SELECT A.S_IDNO,
		A.MTH_DATE,
		CONVERT(FLOAT, B.TOTAL_DAILY)/(CONVERT(FLOAT, B.TOTAL_DAILY) + CONVERT(FLOAT, A.TOTAL_MATCH)) AS DAY_TRADE_RATIO
FROM(
SELECT MATCH_DAY.MTH_DATE AS MTH_DATE,
        CUSTOMER_INFO.S_IDNO AS S_IDNO,
		COUNT(*) AS TOTAL_MATCH
...
INNER JOIN TRADE_DAILY ON STAT_DAY.S_ACNO = TRADE_DAILY.S_ACNO AND STAT_DAY.DATE_YMD = TRADE_DAILY.MTH_DATE AND STAT_DAY.BROKER_NO4 = TRADE_DAILY.BROKER_NO4
GROUP BY TRADE_DAILY.MTH_DATE, CUSTOMER_INFO.S_IDNO) B
ON A.MTH_DATE = B.MTH_DATE AND A.S_IDNO = B.S_IDNO)C
...
SELECT *
FROM ODS.dbo.ST_DAT200_D
WHERE BROKER_NO4_EX = '9A9P' AND S_ACNO = '0200926' AND DATE_YMD = '20191230'

SELECT *
FROM ODS.dbo.ST_MATCH_D
WHERE BROKER_NO4_EX = '9A9P' AND S_ACNO = '0200926' AND MTH_DATE = '20191230'

SELECT S_ACNO, S_IDNO, BROKER_NO4_EX
FROM ODS.dbo.ST_CUSTOMER
WHERE S_IDNO = 'J222022332'