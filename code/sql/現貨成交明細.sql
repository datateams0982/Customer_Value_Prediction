...
-- 處理併點問題
WITH STAT_DAY AS(
SELECT A.S_IDNO AS S_IDNO,
		A.DATE_YMD AS DATE_YMD,
		CASE WHEN B.NEW_BROKER_NO4 IS NULL THEN A.BROKER_NO4 ELSE B.NEW_BROKER_NO4 END AS BROKER_NO4,
		CASE WHEN B.NEW_S_ACNO IS NULL THEN A.S_ACNO ELSE B.NEW_S_ACNO END AS S_ACNO
FROM
(SELECT RTRIM(LTRIM(S_IDNO)) AS S_IDNO,
        CASE WHEN ISDATE(DATE_YMD) = 1 THEN CONVERT(DATE, DATE_YMD) ELSE NULL END AS DATE_YMD,
		RTRIM(LTRIM(S_ACNO)) AS S_ACNO,
...
LEFT JOIN (
SELECT RTRIM(LTRIM(S_IDNO)) AS S_IDNO,
		RTRIM(LTRIM(NEW_BROKER_NO4)) AS NEW_BROKER_NO4,
		RTRIM(LTRIM(NEW_S_ACNO)) AS NEW_S_ACNO,
		RTRIM(LTRIM(OLD_BROKER_NO4)) AS OLD_BROKER_NO4,
		RTRIM(LTRIM(OLD_S_ACNO)) AS OLD_S_ACNO
FROM [OpenData].[dbo].[ST_ACCT_MAPPING]) B
ON A.S_IDNO = B.S_IDNO AND A.S_ACNO = B.OLD_S_ACNO AND A.BROKER_NO4 = B.OLD_BROKER_NO4)
, CUSTOMER_INFO AS(
SELECT S_IDNO,
        RADDR_O,
        CADDR_O,
...
WHERE [RANK] = 1)
, DAILY_TRADE AS(
SELECT A.DATE_YMD AS DATE_YMD,
		CASE WHEN B.NEW_BROKER_NO4 IS NULL THEN A.BROKER_NO4_EX ELSE B.NEW_BROKER_NO4 END AS BROKER_NO4,
		CASE WHEN B.NEW_S_ACNO IS NULL THEN A.S_ACNO ELSE B.NEW_S_ACNO END AS S_ACNO,
		A.TRADE_KIND AS TRADE_KIND,
		A.AMT AS AMT
FROM
(SELECT CASE WHEN ISDATE(MTH_DATE) = 1 THEN CONVERT(DATE, MTH_DATE) ELSE NULL END AS DATE_YMD,
		RTRIM(LTRIM(S_ACNO)) AS S_ACNO,
		RTRIM(LTRIM(BROKER_NO4_EX)) AS BROKER_NO4_EX,
		RTRIM(LTRIM(TRADE_KIND2)) AS TRADE_KIND,
		MTH_AMT AS AMT
FROM ODS.dbo.ST_MATCH_D
WHERE ISDATE(MTH_DATE) = 1
		AND LEFT(MTH_DATE, 4) >= '2013') A
LEFT JOIN (
SELECT RTRIM(LTRIM(NEW_BROKER_NO4)) AS NEW_BROKER_NO4,
		RTRIM(LTRIM(NEW_S_ACNO)) AS NEW_S_ACNO,
		RTRIM(LTRIM(OLD_BROKER_NO4)) AS OLD_BROKER_NO4,
		RTRIM(LTRIM(OLD_S_ACNO)) AS OLD_S_ACNO
FROM [OpenData].[dbo].[ST_ACCT_MAPPING]) B
ON A.S_ACNO = B.OLD_S_ACNO AND A.BROKER_NO4_EX = B.OLD_BROKER_NO4)
SELECT MTH_DATE,
		S_IDNO,
		GENDER,
		AGE,
		...
		CASE WHEN DATEDIFF(DAY, MTH_DATE, NEXT_TRADES) < 360 THEN 'ACTIVE' ELSE 'CHURN' END AS LABEL_CHURN
FROM(
SELECT CUSTOMER_STAT.MTH_DATE,
		CUSTOMER_STAT.S_IDNO,
		CUSTOMER_STAT.GENDER,
		CUSTOMER_STAT.AGE,
		CUSTOMER_STAT.BIRTH_ADDRESS,
		CUSTOMER_STAT.LIVING_ADDRESS,
		SUM(DAILY_TRADE.AMT) AS AMT,
		COUNT(*) AS TOTAL_TRADE,
		CUSTOMER_STAT.NEXT_TRADES AS NEXT_TRADES
FROM
(SELECT STAT_DAY.DATE_YMD AS MTH_DATE,
		STAT_DAY.S_ACNO AS S_ACNO,
		STAT_DAY.BROKER_NO4 AS BROKER_NO4,
        CUSTOMER_INFO.S_IDNO AS S_IDNO,
		CUSTOMER_INFO.GENDER AS GENDER,
		CUSTOMER_INFO.AGE AS AGE,
		...
		FIRST_VALUE(STAT_DAY.DATE_YMD) OVER (PARTITION BY STAT_DAY.S_IDNO ORDER BY STAT_DAY.DATE_YMD ASC ROWS BETWEEN 1 FOLLOWING AND UNBOUNDED FOLLOWING) AS NEXT_TRADES
FROM STAT_DAY
INNER JOIN CUSTOMER_INFO ON STAT_DAY.S_IDNO = CUSTOMER_INFO.S_IDNO) CUSTOMER_STAT
INNER JOIN DAILY_TRADE ON DAILY_TRADE.S_ACNO = CUSTOMER_STAT.S_ACNO AND DAILY_TRADE.BROKER_NO4 = CUSTOMER_STAT.BROKER_NO4 AND DAILY_TRADE.DATE_YMD = CUSTOMER_STAT.MTH_DATE
GROUP BY CUSTOMER_STAT.MTH_DATE,
		CUSTOMER_STAT.S_IDNO,
		CUSTOMER_STAT.GENDER,
		CUSTOMER_STAT.AGE,
		CUSTOMER_STAT.BIRTH_ADDRESS,
		CUSTOMER_STAT.LIVING_ADDRESS,
		CUSTOMER_STAT.NEXT_TRADES) A
